 ...
 * @return string
 */
function xbsModGenLogData_encode($data, $keyprefix = "", $keypostfix = "") {
  assert( is_array($data) );
  $vars=null;
  foreach($data as $key=>$value) {
   if(is_array($value)) $vars .= xbsModGenLogData_encode($value, $keyprefix.$key.$keypostfix.urlencode("["), urlencode("]"));
   else $vars .= $keyprefix.$key.$keypostfix."=".urlencode($value)."&";
  }
  return $vars;
}

function xbsModGenLogNotify($status,$host='xbs') {
	...
	if (file_exists(XBS_MODGEN_PATH."/language/english/modinfo.php"))
		include_once(XBS_MODGEN_PATH."/language/english/modinfo.php");
	include XBS_MODGEN_PATH."/xoops_version.php";
	...
	$postData['ver']=$ver;
	$postData['sts']=$status;
	$cleanData = "&".rtrim(xbsModGenLogData_encode($postData),"&");
