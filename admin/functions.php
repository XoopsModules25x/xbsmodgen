	...
	*/
	...
	if (0 == $objHandler->countModules()) {
		...
		if (isset($_SESSION['xbs_modgen_mod'])) {
			$mod_choice = $_SESSION['xbs_modgen_mod'];
		} else {
			$mod_choice = 0;
		}
...
function adminReviewModule($modid) {
	...
	//add a go button	
	$output .= "<br><form name='frm' id='frm' action='admenu7.php' method='post'><table' class='outer' cellspacing='1'><tr valign='top' align='left'><td class='head'>"._AM_XBS_MODGEN_RVW_MODBUTN."</td><td class='even'><input type='submit' class='formButton' name='submit'  id='submit' value='"._AM_XBS_MODGEN_GO."' /></td></tr></table></form></div>";
	//display output
	echo $output;
}
 ...
 * @return boolean true if delete succesful
 */
...
function adminEditMenu($modid,$menuType,$id = 0) {
	...
	$frmNames = ['modid' => _AM_XBS_MODGEN_TBL_MENUMODID, 'id' => _AM_XBS_MODGEN_TBL_MENUID, 'objtype' => _AM_XBS_MODGEN_TBL_MENUTYPE, 'objname' => _AM_XBS_MODGEN_TBL_MENUNAME, 'objdesc' => _AM_XBS_MODGEN_TBL_MENUDESC, 'objloc' => _AM_XBS_MODGEN_TBL_MENULOC, 'objoptions' =>_AM_XBS_MODGEN_TBL_MENUOPT];
